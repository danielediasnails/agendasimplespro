...
const App: React.FC = () => {
  ...
  const [showInternalAlert, setShowInternalAlert] = useState(false);
  ...
  const playAlertSound = () => {
    try {
      ...
      audioRef.current.play().catch(e => {
        console.warn("Reprodução automática de áudio bloqueada pelo navegador.");
      });
    } catch (e) {
      console.warn("Não foi possível carregar o som de alerta.");
    }
  };
  ...
  // Lógica do Timer de Notificação
  useEffect(() => {
    if (!notifEnabled || !auth.isAuthenticated || auth.role !== 'master') return;

    const interval = setInterval(() => {
      const now = new Date();
      const currentHM = `${now.getHours().toString().padStart(2, '0')}:${now.getMinutes().toString().padStart(2, '0')}`;
      
      if (currentHM === notifTime && lastNotifiedDay !== todayStr) {
        // Disparar Notificação Interna
        setLastNotifiedDay(todayStr);
    ...
    google.script.run.withSuccessHandler((data: any) => {
      if (data && !data.error) {
        if (data.settings) {
          ...
          setNotifEnabled(data.settings.notifEnabled === 'true');
          setNotifTime(data.settings.notifTime || '10:00');
          
          const loadedStandard = data.settings.userTimes ? data.settings.userTimes.split(',').filter(Boolean).sort() : STANDARD_TIMES;
          const loadedFreeRaw = data.settings.freeTimes ? data.settings.freeTimes.split(',').filter(Boolean).sort() : FREE_TIMES;
          // Garante que o alternativo não tenha o que já existe no padrão (prioridade para o padrão)
          const loadedFree = loadedFreeRaw.filter(t => !loadedStandard.includes(t));
          
          setUserTimes(loadedStandard);
          setUserFreeTimes(loadedFree);
  ...
  const tomorrowAppointments = useMemo(() => {
    let base = appointments || [];
    if (auth.role === 'partner') base = base.filter(app => app.partnerName === auth.username);
    return base.filter(app => app.date === tomorrowStr).sort((a, b) => a.time.localeCompare(b.time));
  ...
  /**
   * CIRÚRGICO: Define se uma data retroativa deve ser considerada "bloqueada/indisponível".
   * Bloqueia apenas se: for passado E (não tiver agendamentos OU todos agendamentos estiverem preenchidos com valor > 0).
   */
  const isDateRetroactiveAndBlocked = useMemo(() => (dateStr: string) => {
    if (dateStr >= todayStr) return false;
    const dayApps = (appointments || []).filter(a => a.date === dateStr);
    if (dayApps.length === 0) return true; // Retroativo sem nada é bloqueado por padrão
    // Se algum atendimento tiver valor total zero, o dia NÃO está bloqueado (está pendente)
    return dayApps.every(a => Number(a.totalValue || 0) > 0);
  ...
  const monthAvailability = useMemo(() => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      // FILTRO: Se o dia já passou e está totalmente finalizado, não deve aparecer na lista de horários disponíveis
      if (isDateRetroactiveAndBlocked(dateStr)) continue;
    ...
    return results;
  }, [calendarViewDate, blockedDays, blockedTimes, appointments, userTimes, userFreeTimes, monthSlotsSearch, todayStr, isDateRetroactiveAndBlocked]);
  ...
  const persistSettings = (opts: any = {}) => {
    const settings = { 
      ...
      notifEnabled: opts.updatedNotifEnabled !== undefined ? String(opts.updatedNotifEnabled) : String(notifEnabled),
      notifTime: opts.updatedNotifTime || notifTime,
  ...
  const handleAddTime = () => {
    ...
    const updatedStandard = [...userTimes, newTime].sort();
    const updatedFree = userFreeTimes.filter(t => t !== newTime);
    setUserTimes(updatedStandard);
    setUserFreeTimes(updatedFree);
    persistSettings({ updatedTimes: updatedStandard, updatedFreeTimes: updatedFree });
  };
  ...
  const handleUpdateStandardTime = () => {
    ...
    const updatedStandard = [...userTimes];
    updatedStandard[editingStandardTimeIdx] = tempTimeEditValue;
    updatedStandard.sort();
    const updatedFree = userFreeTimes.filter(t => t !== tempTimeEditValue);
    setUserTimes(updatedStandard);
    setUserFreeTimes(updatedFree);
  ...
  const handleAddFreeTime = () => {
    ...
    if (userTimes.includes(newFreeTime)) {
      alert("Este horário já existe na lista padrão.");
      return;
    }
  ...
  const handleUpdateFreeTime = () => {
    ...
    if (userTimes.includes(tempTimeEditValue)) {
      alert("Este horário já existe na lista padrão.");
      return;
    }
  ...
  // CIRÚRGICO: Bloqueia novo agendamento na data retroativa apenas se estiver finalizada.
  const isSelectedDateRetroactiveBlocked = isDateRetroactiveAndBlocked(selectedDate);

  const handleFastConfirmation = (app: Appointment) => {
    ...
    const dayOfWeek = getDayOfWeek(app.date);
    const formattedTime = formatTime(app.time);
    const message = `Olá, posso confirmar seu agendamento (${d}/${m} ${dayOfWeek} ${formattedTime})?`;
  ...
  return (
    <div className="min-h-screen bg-inherit text-inherit pb-24 transition-colors duration-500">
      ...
      {/* Alerta de Notificação Interno - Corrigido Layout e Som em Loop */}
      {showInternalAlert && (
        <div 
          onClick={() => { stopAlertSound(); setShowInternalAlert(false); setIsNextDayConfirmOpen(true); }}
          className="fixed top-0 left-0 right-0 z-[100] animate-flash-green-red py-6 px-4 shadow-2xl cursor-pointer animate-in slide-in-from-top duration-1000 overflow-hidden border-b-2 border-white/30"
        >
           {/* Container centralizador com padding lateral extra para evitar colisão com os ícones absolutos */}
           <div className="relative max-w-5xl mx-auto flex items-center justify-center min-h-[48px]">
              <div className="absolute left-0">
                <Bell size={32} className="text-white shrink-0" />
              </div>

              <div className="flex flex-col items-center text-center px-16">
                 <span className="text-[14px] sm:text-[16px] font-black uppercase tracking-[0.2em] text-white whitespace-normal break-words drop-shadow-md leading-tight">
                   CONFIRMAR CLIENTES
                 </span>
                 <span className="text-[9px] sm:text-[10px] font-bold text-white/95 uppercase tracking-widest leading-none mt-2">
                   (aperte aqui para confirmar)
                 </span>
              </div>

              <button 
                onClick={(e) => { e.stopPropagation(); stopAlertSound(); setShowInternalAlert(false); }}
                className="absolute right-0 bg-white text-red-600 rounded-full p-2 shadow-lg hover:bg-slate-50 transition-all active:scale-90"
              >
                <X size={24} />
              </button>
           </div>
      ...
      <main className="max-w-5xl mx-auto p-6 space-y-8">
        ...
        {auth.role === 'master' && (
          <div className="space-y-3">
            <button 
              onClick={() => { 
                if(isDayBlocked || isSelectedDateRetroactiveBlocked){ 
                  alert(isDayBlocked ? 'Este dia está bloqueado.' : 'Este dia está finalizado e não aceita novos agendamentos.'); 
                  return; 
                } 
                setEditingAppointment(null); 
              ...
              className={`w-full text-white font-bold px-4 py-5 rounded-2xl shadow-lg flex items-center justify-center gap-2 uppercase tracking-tight text-sm hover:scale-[1.02] active:scale-95 transition-all group whitespace-nowrap ${isDayBlocked || isSelectedDateRetroactiveBlocked ? 'bg-slate-300 dark:bg-slate-800 cursor-not-allowed grayscale' : 'gold-gradient'}`} 
              disabled={isDayBlocked || isSelectedDateRetroactiveBlocked}
            >
              <Plus size={24} />
             ...
             <div className="p-4 sm:p-6">
                ...
                <div className="grid grid-cols-7 gap-2 sm:gap-4">
                  {monthDays.map((dayNum, i) => {
                    ...
                    // CIRÚRGICO: Define se o dia deve estar visualmente desabilitado
                    const isRetroactiveBlocked = isDateRetroactiveAndBlocked(key);
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex gap-1 mt-1">
                          {calIsBlocked && (
                            <div className={`w-1.5 h-1.5 rounded-full bg-red-500 ${calIsSelected ? 'ring-2 ring-white' : ''}`} />
                          )}
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] w-full max-w-2xl shadow-2xl overflow-hidden border border-slate-100 dark:border-slate-800 animate-in zoom-in-95 flex flex-col max-h-[90vh]">
            <div className="p-8 border-b dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50 shrink-0">
               <div className="flex flex-col">
                  <h3 className="text-xl font-black uppercase tracking-tight">CONFIRMAR CLIENTE</h3>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">
                    {tomorrowStr.split('-').reverse().join('/')} ({['Domingo', 'Segunda-feira', 'Terça-feira', 'Quarta-feira', 'Quinta-feira', 'Sexta-feira', 'Sábado'][new Date(tomorrowStr + 'T12:00:00').getDay()]})
                  </p>
               </div>
               <button onClick={() => setIsNextDayConfirmOpen(false)} className="p-2 text-slate-400 hover:scale-110 transition-transform"><X size={24} /></button>
            </div>
            
            <div className="flex-1 p-8 overflow-y-auto custom-scrollbar space-y-6">
               <div className="bg-amber-50 dark:bg-amber-950/20 p-4 rounded-2xl border-2 border-dashed border-amber-200 dark:border-amber-900/40 flex gap-3 items-center">
                  <AlertCircle size={20} className="text-amber-500 shrink-0" />
                  <p className="text-[10px] font-bold text-amber-700 dark:text-amber-400 uppercase tracking-wider">Envie as confirmações para as clientes agendadas para amanhã.</p>
               </div>

               {tomorrowAppointments.length > 0 ? (
                 <div className="grid grid-cols-1 gap-4">
                    {tomorrowAppointments.map(app => (
                      <div key={app.id} className="p-4 rounded-[2rem] border-2 border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm transition-all hover:border-[var(--primary-color)] group flex items-center justify-between gap-4">
                         <div className="flex items-center gap-4 min-w-0">
                            <div className="w-12 h-12 rounded-2xl gold-gradient flex flex-col items-center justify-center text-white shrink-0 shadow-sm">
                               <Clock size={16} />
                               <span className="text-[10px] font-black mt-0.5">{app.time.replace(':', 'h')}</span>
                            </div>
                            <div className="min-w-0">
                               <p className="text-sm font-black uppercase tracking-tight truncate gold-text-gradient">{app.clientName}</p>
                               <div className="flex flex-col gap-0.5 mt-0.5">
                                  <div className="flex items-center gap-1.5">
                                     <Sparkles size={10} className="text-slate-400" />
                                     <span className="text-[10px] font-bold text-slate-500 uppercase truncate">{app.procedure}</span>
                                  </div>
                                  {app.secondaryProcedure && (
                                    <div className="flex items-center gap-1.5 border-t border-slate-50 dark:border-slate-800 pt-0.5">
                                       <Sparkle size={10} className="text-emerald-500" />
                                       <span className="text-[10px] font-bold text-emerald-600/70 uppercase truncate">{app.secondaryProcedure}</span>
                                    </div>
                                  )}
                               </div>
                            </div>
                         ...
                         <button 
                           onClick={() => handleFastConfirmation(app)}
                           className="flex items-center justify-center p-4 bg-emerald-500 text-white rounded-2xl shadow-lg active:scale-95 transition-all shrink-0 hover:scale-110"
               ...
               {monthAvailability.map((day) => {
                 ...
                 <div key={day.date} className="space-y-4 animate-in fade-in duration-300">
                    <div className="flex items-center gap-4 sticky top-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm py-2 z-10">
                       <div className={`w-12 h-12 rounded-2xl flex flex-col items-center justify-center text-white shadow-md shrink-0 ${day.date < todayStr ? 'bg-slate-400' : 'gold-gradient'}`}>
                       ...
                       <span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em]">{day.isBlocked ? 'BLOQUEADO' : day.date < todayStr ? 'CONCLUÍDO' : `${day.available.standard.length + day.available.free.length} VAGOS`}</span>
                    </div>

                    {!day.isBlocked && day.date >= todayStr && (
          ...
          <div className="bg-white dark:bg-slate-900 rounded-l-[2.5rem] rounded-r-none w-full max-lg shadow-2xl p-6 sm:p-8 space-y-8 max-h-[90vh] overflow-y-auto custom-scrollbar pr-2 animate-in zoom-in-95">
            ...
            <div className="space-y-10">
              ...
              <div className="space-y-6">
                ...
                {!isSelectedDateRetroactiveBlocked ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-6 text-slate-400 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border-2 border-dashed">
          ...
          <div className="relative w-full max-w-[300px] bg-white dark:bg-slate-900 h-full shadow-2xl p-8 flex flex-col">
            ...
            <nav className="space-y-4 flex-1 overflow-y-auto custom-scrollbar pr-4">
              {auth.role === 'master' ? (
                <>
                  {[
                    { label: 'CONFIRMAR CLIENTE', icon: <Send size={20} />, action: () => setIsNextDayConfirmOpen(true) },
                    { label: 'Clientes', icon: <Users size={20} />, action: () => setIsClientsOpen(true) },
                    { label: 'Histórico', icon: <HistoryIcon size={20} />, action: () => setIsHistoryOpen(true) },
                    { label: 'Procedimentos', icon: <Sparkle size={20} />, action: () => setIsProceduresOpen(true) },
                    { label: 'Horários', icon: <Clock size={20} />, action: () => setIsTimesOpen(true) },
                    { label: 'Despesas', icon: <ShoppingBag size={20} />, action: () => setIsExpensesOpen(true) },
                    { label: 'Financeiro', icon: <Briefcase size={20} />, action: () => setIsFinanceOpen(true) },
                    { label: 'Parceiras', icon: <HardHat size={20} />, action: () => setIsPartnersOpen(true) },
                    { label: 'Ajustes', icon: <Settings size={20} />, action: () => setIsSettingsOpen(true) }
                  ].map((item, idx) => (
                    <button key={idx} onClick={() => { item.action(); setIsMenuOpen(false); }} className={`w-full flex items-center gap-3 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 ${item.label === 'CONFIRMAR CLIENTE' ? 'bg-emerald-50 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-800' : 'bg-slate-50 dark:bg-slate-800/50'} text-slate-700 dark:text-slate-200 hover:border-[var(--primary-color)] font-bold uppercase tracking-wider text-[10px]`}>
                      <div className={`p-2 rounded-xl bg-white dark:bg-slate-800 shadow-sm ${item.label === 'CONFIRMAR CLIENTE' ? 'text-emerald-500' : 'text-[var(--primary-color)]'} shrink-0`}>{item.icon}</div>
                ...
                <>
                  ...
                  <button onClick={() => { setIsFinanceOpen(true); setIsMenuOpen(false); }} className="w-full flex items-center gap-3 p-4 rounded-2xl border border-slate-100 bg-slate-50 text-slate-700 font-bold uppercase tracking-wider text-[10px]"><div className="p-2 bg-white shadow-sm text-[var(--primary-color)] shrink-0"><Briefcase size={22} /></div><span>Meu Financeiro</span></button>
                </>
              )}
              <button onClick={handleLogout} className="w-full flex items-center gap-3 p-4 rounded-2xl bg-red-50 text-red-600 mt-8 border border-red-100 font-bold text-[10px] uppercase tracking-wider"><LogOut size={22} /><span>Sair</span></button>
            </nav>
          </div>
        </div>
      )}
      ...
      {isSettingsOpen && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 sm:p-6 bg-slate-900/40 backdrop-blur-md">
          <div className="bg-white dark:bg-slate-900 rounded-l-[2.5rem] rounded-r-none w-full max-lg shadow-2xl p-6 sm:p-8 space-y-8 max-h-[90vh] overflow-y-auto custom-scrollbar pr-2">
            ...
            <div className="space-y-4">
               <div className="flex items-center gap-2 ml-1">
                 <BellRing size={16} className="text-[var(--primary-color)]" />
                 <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Lembretes de Confirmação</h4>
               </div>
               <div className="bg-slate-50 dark:bg-slate-800/30 p-5 rounded-[1.5rem] border border-slate-100 dark:border-slate-800 space-y-6">
                  <div className="flex items-center justify-between">
                    <div className="space-y-0.5">
                      <label className="text-[10px] font-black text-slate-700 dark:text-slate-200 uppercase tracking-tight">Ativar Alertas</label>
                      <p className="text-[8px] font-bold text-slate-400 uppercase">Avisar diariamente o envio</p>
                    </div>
                    <button 
                      onClick={() => {
                        const nextVal = !notifEnabled;
                        setNotifEnabled(nextVal);
                        // No mobile, evitamos usar o Notification.requestPermission para manter tudo apenas interno se preferível
                        // Mas deixamos a lógica base para desktop se necessário
                      }}
                      className={`w-14 h-7 rounded-full transition-all relative p-1 shadow-inner ${notifEnabled ? 'bg-emerald-500' : 'bg-slate-200 dark:bg-slate-700'}`}
                    >
                      <div className={`w-5 h-5 bg-white rounded-full shadow-md transition-transform duration-300 ${notifEnabled ? 'translate-x-7' : 'translate-x-0'}`} />
                    </button>
                  </div>

                  {notifEnabled && (
                    <div className="space-y-2 animate-in slide-in-from-top-2 duration-300">
                      <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Horário do Alerta</label>
                      <div className="relative">
                        ...
                        <input 
                          ...
                          className="w-full bg-white dark:bg-slate-950 p-3 pl-10 rounded-xl text-sm font-bold border outline-none focus:ring-2 ring-emerald-500" 
                        />
                      </div>
                      <p className="text-[8px] font-bold text-emerald-600/70 dark:text-emerald-400/50 uppercase tracking-widest mt-2 ml-1 flex items-center gap-1.5">
                         <ShieldCheck size={10} /> Notificação configurada para às {notifTime}
                      </p>
                    </div>
                  )}
               </div>
