...
const App: React.FC = () => {
  ...
  const initCropper = () => {
    if (cropImageRef.current) {
      ...
      cropperRef.current = new Cropper(cropImageRef.current, {
        aspectRatio: 1, // TRAVA CIRÚRGICA: Impede deformação oval
        viewMode: 1,
        dragMode: 'move',
        cropBoxMovable: false, // Trava a moldura no centro
        cropBoxResizable: false, // Força a proporção 1:1
        background: false,
        autoCropArea: 0.8, // Margem de segurança para o círculo não encostar na borda
        zoomable: true,
        scalable: false,
  ...
  const handleSaveCrop = () => {
    if (cropperRef.current) {
      ...
      // Criar canvas auxiliar para recorte circular final
      ...
      <div className="w-full max-sm space-y-8 animate-in fade-in zoom-in-95 duration-700 flex flex-col items-center">
        <div className="flex flex-col items-center text-center">
          <div className="text-black mb-6">
             {/* CIRÚRGICO: Mantém ícone original no login independente de logo interna */}
  ...
  return (
    <div className="min-h-screen bg-inherit text-inherit pb-24 transition-colors duration-500">
      ...
      <main className="max-w-5xl mx-auto p-6 space-y-8">
        ...
        {auth.role === 'master' && (
          <button onClick={() => { if(isDayBlocked){ alert('Este dia está bloqueado.'); return; } setEditingAppointment(null); setPreselectedTime(null); setIsFormOpen(true); }} className={`w-full text-white font-bold px-6 py-5 rounded-2xl shadow-lg flex items-center justify-center gap-3 uppercase tracking-[0.2em] text-sm ${isDayBlocked ? 'bg-slate-300 dark:bg-slate-800 cursor-not-allowed grayscale' : 'gold-gradient'}`} disabled={isDayBlocked}><Plus size={24} /><span>Novo Agendamento</span></button>
        )}
        ...
        {!searchQuery.trim() && (
          <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden animate-in fade-in duration-700">
             ...
             <div className="border-t dark:border-slate-800">
                ...
                <div className="p-6 bg-white dark:bg-slate-900 flex items-center justify-around">
                  <div className="flex flex-col items-center gap-3">
                    <div className="w-16 h-16 rounded-full bg-slate-50 dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 flex flex-col items-center justify-center shadow-inner group transition-all hover:border-[var(--primary-color)]/30">
                       <span className="text-xl font-black text-[var(--primary-color)] leading-none">{(filteredAppointments || []).length}</span>
                    </div>
                    <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Agendamentos</span>
                  </div>
                  ...
                  {auth.role === 'master' && (
                    <div className="flex flex-col items-center gap-3">
                      <button 
                        ...
                        className={`w-16 h-16 rounded-full flex items-center justify-center transition-all border-2 shadow-sm ${
                          isDayBlocked 
                      ...
                      <span className={`text-[10px] font-black uppercase tracking-widest ${isDayBlocked ? 'text-red-500' : 'text-slate-400'}`}>
                        {isDayBlocked ? 'Desbloquear' : 'Bloquear'}
                      </span>
                    </div>
                  )}
                </div>
        ...
        {!searchQuery.trim() && (
          <div className="space-y-10 animate-in fade-in slide-in-from-top-4">
            ...
            {auth.role === 'master' && (slotsStatus.occupied.standard.length > 0 || slotsStatus.occupied.free.length > 0) && (
              <div className="space-y-6">
                ...
                <div className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden p-6 sm:p-8">
                  <div className="grid grid-cols-2 gap-0">
                    <div className="pr-4 sm:pr-8 border-r border-slate-100 dark:border-slate-800 space-y-5">
                      <div className="flex items-center gap-2 mb-2">
                        <div className="p-1.5 rounded-lg bg-[var(--primary-color)]/10 text-[var(--primary-color)]">
                          <Clock size={12} />
                        </div>
                        <p className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest">Padrão</p>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {slotsStatus.occupied.standard.length > 0 ? slotsStatus.occupied.standard.map(time => (
                          <div key={time} className="px-3 py-1.5 sm:px-4 sm:py-2.5 rounded-xl border-2 border-dashed border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950 text-slate-300 font-bold text-[11px] sm:text-sm uppercase line-through">
                            {time.includes(':00') ? time.split(':')[0] + 'h' : time.replace(':', 'h')}
                          </div>
                        )) : (
                          <span className="text-[8px] text-slate-200 uppercase font-black tracking-widest italic py-4">Vazio</span>
                        )}
                      </div>
                    </div>

                    <div className="pl-4 sm:pl-8 space-y-5">
                      <div className="flex items-center gap-2 mb-2">
                        <div className="p-1.5 rounded-lg bg-emerald-500/10 text-emerald-500">
                          <Clock size={12} />
                        </div>
                        <p className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest">Alternativo</p>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {slotsStatus.occupied.free.length > 0 ? (isOccupiedFreeTimesExpanded ? slotsStatus.occupied.free : slotsStatus.occupied.free.slice(0, 6)).map(time => (
                          <div key={time} className="px-3 py-1.5 sm:px-4 sm:py-2.5 rounded-xl border-2 border-dashed border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950 text-slate-300 font-bold text-[11px] sm:text-sm uppercase line-through">
                            {time.includes(':00') ? time.split(':')[0] + 'h' : time.replace(':', 'h')}
                          </div>
                        )) : (
                      ...
                      {slotsStatus.occupied.free.length > 6 && (
                        <button 
                          onClick={() => setIsOccupiedFreeTimesExpanded(!isOccupiedFreeTimesExpanded)}
                          className="text-[8px] font-black text-slate-400 uppercase tracking-widest px-3 py-1.5 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg transition-all border border-slate-100 dark:border-slate-800 flex items-center gap-1.5"
                        >
                          {isOccupiedFreeTimesExpanded ? 'Recolher' : `+ ${slotsStatus.occupied.free.length - 6} mais`}
                        </button>
                      )}
                    </div>
                  </div>
            ...
            {auth.role === 'master' && !isDayBlocked && (slotsStatus.available.standard.length > 0 || slotsStatus.available.free.length > 0) && (
              <div className="space-y-6">
                <div className="flex items-center gap-4 px-1">
                    ...
                    <div className="pr-4 sm:pr-8 border-r border-slate-100 dark:border-slate-800 space-y-5">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {slotsStatus.available.standard.length > 0 ? slotsStatus.available.standard.map(time => (
                          <button key={time} onClick={() => handleOpenAvailableTime(time)} className="px-3 py-1.5 sm:px-4 sm:py-2.5 rounded-xl border-2 border-dashed border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-500 font-bold hover:border-[var(--primary-color)] hover:text-[var(--primary-color)] transition-all text-[11px] sm:text-sm uppercase shadow-sm active:scale-95">
                            {time.includes(':00') ? time.split(':')[0] + 'h' : time.replace(':', 'h')}
                          </button>
                        )) : (
                          <span className="text-[8px] text-slate-200 uppercase font-black tracking-widest italic py-4">Sem horários</span>
                        )}
                      </div>
                    </div>
      ...
      {isPartnersOpen && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 sm:p-6 bg-slate-900/40 backdrop-blur-md">
          <div className="bg-white dark:bg-slate-900 rounded-l-[2.5rem] rounded-r-none w-full max-w-2xl shadow-2xl p-6 sm:p-8 space-y-8 max-h-[90vh] overflow-y-auto custom-scrollbar">
            <div className="flex justify-between items-center shrink-0">
               <div className="flex flex-col">
      ...
      {isClientsOpen && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-md">
          <div className="bg-white dark:bg-slate-900 rounded-l-[2.5rem] rounded-r-none w-full max-w-2xl shadow-2xl p-8 sm:p-10 space-y-8 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center">
              <div className="flex flex-col">
      ...
      {isHistoryOpen && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 sm:p-6 bg-slate-900/40 backdrop-blur-md">
          <div className="bg-white dark:bg-slate-900 rounded-l-[2.5rem] rounded-r-none w-full max-w-4xl shadow-2xl p-6 sm:p-10 flex flex-col max-h-[90vh]">
      ...
      {isProceduresOpen && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 sm:p-6 bg-slate-900/40 backdrop-blur-md">
          <div className="bg-white dark:bg-slate-900 rounded-l-[2.5rem] rounded-r-none w-full max-w-2xl shadow-2xl p-6 sm:p-8 space-y-6 max-h-[90vh] overflow-y-auto custom-scrollbar">
            <div className="flex justify-between items-center shrink-0">
            ...
            <div className="space-y-3 pt-2 border-t dark:border-slate-800 px-2">
               ...
               <div className="bg-slate-50 dark:bg-slate-800/30 p-4 rounded-[1.5rem] border space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {/* FIXED: Using correct state variable and setter for expense payment method */}
