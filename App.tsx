...
const App: React.FC = () => {
    ...
    const interval = setInterval(() => {
      ...
      if (currentHM === notifTime && lastNotifiedDay !== todayStr) {
        ...
        if ("Notification" in window && Notification.permission === "granted") {
          new Notification("Agenda Simples: CONFIRMAR CLIENTE", {
            body: "É hora de enviar as confirmações para as clientes de amanhã!",
  ...
  const groupedHistory = useMemo(() => {
    ...
    // ORDENAÇÃO ALTERADA: a.localeCompare(b) para ordem crescente (dia 1 até o último)
  ...
  return (
    <div className="min-h-screen bg-inherit text-inherit pb-24 transition-colors duration-500">
      ...
      {/* Alerta de Notificação Interno */}
      {showInternalAlert && (
        <div className="fixed top-24 left-1/2 -translate-x-1/2 z-[100] w-full max-w-sm px-6 animate-in slide-in-from-top-10 duration-500">
           <div className="bg-white dark:bg-slate-900 border-2 border-emerald-500 shadow-[0_20px_50px_rgba(16,185,129,0.3)] rounded-[2rem] p-6 flex flex-col items-center text-center gap-4">
              <div className="w-16 h-16 rounded-full bg-emerald-50 dark:bg-emerald-950 flex items-center justify-center text-emerald-500">
                 <BellRing size={32} className="animate-bounce" />
              </div>
              <div>
                <h4 className="text-base font-black uppercase tracking-tight text-emerald-600">CONFIRMAR CLIENTE</h4>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Hora de enviar as confirmações de amanhã!</p>
              </div>
              <div className="flex gap-2 w-full">
                <button 
                  onClick={() => { setShowInternalAlert(false); setIsNextDayConfirmOpen(true); }}
                  className="flex-1 py-4 bg-emerald-500 text-white rounded-xl font-bold uppercase text-[10px] tracking-widest active:scale-95 transition-all"
                >
                  Abrir Agora
                </button>
                <button 
                  onClick={() => setShowInternalAlert(false)}
                  className="px-6 py-4 bg-slate-100 dark:bg-slate-800 text-slate-400 rounded-xl font-bold uppercase text-[10px] tracking-widest active:scale-95 transition-all"
                >
                  Fechar
                </button>
              </div>
