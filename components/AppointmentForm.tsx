...
interface AppointmentFormProps {
  ...
  availableSecondaryProcedures: string[];
...
const AppointmentForm: React.FC<AppointmentFormProps> = ({ 
  ...
  // Função robusta de verificação de disponibilidade
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6 max-h-[75vh] overflow-y-auto custom-scrollbar">
          ...
          <div className="space-y-4 pt-4 border-t border-slate-50 dark:border-slate-800">
            <div className="flex items-center mb-2">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Procedimento Secundário</span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                <div className="relative">
              ...
              {secondaryProcedure && (
                <div className="space-y-2 animate-in fade-in slide-in-from-top-2">
                  ...
                  {hasConflictSecondary && (
                    <p className="text-[9px] font-bold text-red-500 uppercase tracking-widest mt-1 ml-1 flex items-center gap-1">
                      <AlertTriangle size={10} /> Horário ocupado ou repetido
                    </p>
                  )}
                </div>
              )}
          ...
          <div className="flex gap-4 pt-6">
            {editingData && onDelete && (
              <button 
                ...
                className="p-5 bg-red-50 dark:bg-red-950/20 text-red-500 rounded-2xl hover:bg-red-100 dark:hover:bg-red-950/40 transition-all active:scale-95"
              >
                <Trash2 size={24} />
              </button>
            )}
            <button 
              ...
              className={`flex-1 text-white py-5 rounded-2xl font-bold uppercase text-xs tracking-[0.2em] shadow-xl transition-all flex items-center justify-center gap-3 ${anyConflict ? 'bg-slate-300 cursor-not-allowed opacity-50' : 'green-gradient hover:scale-[1.02] active:scale-95'}`}
            >
